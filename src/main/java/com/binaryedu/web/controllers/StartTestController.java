...
public class StartTestController extends SimpleFormController
{
	...
	{
		logger.info("Returning start test View");
		
		if(!SessionUtil.isUserLoggedIn(request.getSession()))
		{
			return new ModelAndView(new RedirectView("login.htm?fwd=startTest.htm?" + request.getQueryString()));
		...
		return new ModelAndView("TestApp", "model", model);
	}
}
